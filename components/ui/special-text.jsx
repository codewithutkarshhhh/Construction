...
/**
 * SpecialText — animated "decrypt/scramble" reveal for a short string.
 *
 * Adapted for this JS project (types stripped from the original .tsx). The
 * text is rendered inside a monospace <span>; on mount (or when scrolled into
 * view) each character cycles through random glyphs before settling on the
 * final letter, giving a terminal-style decode effect.
 *
 * Props:
 *   children  {string}  The text to animate (required, must be a plain string).
 *   speed     {number}  Interval between animation frames in ms (default 20).
 *   delay     {number}  Delay before the animation starts, in seconds (default 0).
 *   className {string}  Extra Tailwind classes for the wrapping <span>.
 *   inView    {boolean} If true, only animate once the element scrolls into view.
 *   once      {boolean} If true (default), the in-view animation runs a single time.
 */
